...
def impact_result(n, keyword):
    ...
    if keyword == 'cost':
        ...
        for i in n.lines.index.tolist():
            s += (n.lines['s_nom_opt'][i] - n.lines['s_nom'][i]) * n.lines['capital_cost'][i]

    else:
        for i in n.generators.index.tolist():
            s += (n.generators['p_nom_opt'][i] - n.generators['p_nom'][i]) * n.generators[keyword + "_f"][i] + sum(
                n.generators_t['p'][i][t] for t in list(n.snapshots)) * n.generators[keyword + "_v"][i]
        for i in n.links.index.tolist():
            s += (n.links['p_nom_opt'][i] - n.links['p_nom'][i]) * n.links[keyword + "_f"][i] + sum(
                n.links_t['p0'][i][t] for t in list(n.snapshots)) * n.links[keyword + "_v"][i]
        for i in n.stores.index.tolist():
            s += (n.stores['e_nom_opt'][i] - n.stores['e_nom'][i]) * n.stores[keyword + "_f"][i] + sum(
                n.stores_t['p'][i][t] for t in list(n.snapshots)) * n.stores[keyword + "_v"][i]
        for i in n.storage_units.index.tolist():
